...
const IngridientList: React.FC<PROP> = ({ type, category }) => {
   ...
   const handleCheckboxChange = (ingredient: { name: string }, isChecked: boolean) => {
      if (isChecked) {
         ...
         dispatch({ type: ACTION.EDIT, payload: { ...state.pizzas[index], [category]: [...state.pizzas[index][category], ingredient.name] } })
      } else {
         const index = state.pizzas.length - 1
         dispatch({ type: ACTION.EDIT, payload: { ...state.pizzas[index], [category]: [] } })
      }
   }
